...
视频生成并保存后，结果页默认从末尾 1 秒均匀截取 4 张原始分辨率 PNG（包含最后一帧）。
可以调整张数（1–12）和末尾秒数，再点击“重新截取并保存图片”；秒数设为 0 时从整段均匀截取。
若所选范围内帧数不足，则只导出实际可用的帧；只截 1 张时取最后一帧。

图片保存在 `outputs/comfyui/<视频任务 ID>_frames/capture_<唯一标识>/`，页面会显示完整路径并提供下载。
每次截取创建新目录，不覆盖之前的图片。默认选中最后一帧；通过“选择下一段首帧”下拉框选图，
点击“使用选中图片作为下一段首帧”，即可回到第一步。原来的时长、FPS、画幅和镜头设置会保留，
旧的分析、回答和提示词会清空，请填写下一段动作并重新分析。已保存的视频和图片仍保留在本地。

截帧依赖 `opencv-python-headless`（已加入 requirements.txt）；截帧失败不会丢失已生成的视频。
...
本项目目前未附带开源许可证，项目代码及相关原创内容默认保留所有权利。未经项目作者明确许可，
不得复制、修改、分发或用于商业用途。如需使用、改编或集成本项目，请先联系项目作者取得授权。

本项目所调用的 Ollama、ComfyUI、LTX-2.5、Z-Image Turbo 及其他第三方模型、组件和工作流，
分别适用其各自的许可证与使用条款；使用者有责任在使用和分发前确认并遵守相关要求。
